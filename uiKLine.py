...
class KLineWidget(KeyWraper):
    ...
    def plotMark(self):
        ...
        for i in range(len(self.listSig)):
            ...
            # 买信号
                ...
                if self.listOpenSig[i] > 0: 
                    arrow = pg.ArrowItem(pos=(i, self.datas[i]['low']),  angle=90, brush=(255, 0, 0), tailLen = 20)
                else:
                    arrow = pg.ArrowItem(pos=(i, self.datas[i]['low']),  angle=90, brush=(255, 0, 0))
            # 卖信号
            elif self.listSig[i] < 0:
                if self.listOpenSig[i] < 0: 
                    arrow = pg.ArrowItem(pos=(i, self.datas[i]['high']), angle=-90, brush=(0, 255, 0), tailLen = 20)
                else:
    ...
    #----------------------------------------------------------------------
    def updateSig(self,sig,openSig = None):
        """刷新买卖信号"""
        self.listSig = sig
        self.listOpenSig = sig if openSig is None else openSig
        self.plotMark()
