...
def peakdetect(y_axis, x_axis = None, lookahead = 200, delta=0, ratio=0.25):
    """
    ...
    for index, (x, y) in enumerate(zip(x_axis[:-lookahead], 
        ...
        #    delta0 = max((max_peaks[-1][1]-min_peaks[-1][1])*ratio,delta)
        
        ####look for max####
        if y <= mx-delta0 and mx != np.Inf:
